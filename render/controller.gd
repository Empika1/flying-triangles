...
func _ready() -> void:
    var shaderMaterial: ShaderMaterial = load("res://render/render.tres")
    var mmAreas: MultiMeshInstance2D = $FTRender/mmAreas
    var mmBorders: MultiMeshInstance2D = $FTRender/mmBorders
    var mmInsides: MultiMeshInstance2D = $FTRender/mmInsides
    render.init(shaderMaterial, mmAreas, mmBorders, mmInsides)
